...
USO
===
    python .claude/scripts/build-report.py <carpeta-del-discovery>
    # p. ej.
    python .claude/scripts/build-report.py discoveries/liberaciones_obra
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python build-report.py <carpeta-del-discovery>", file=sys.stderr)
        sys.exit(1)
    ...
    sys.stdout.buffer.write(f"[OK] Reporte generado: {path}\n".encode("utf-8"))
